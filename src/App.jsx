...
function Welcome({ onStart }) {
  return <div className="auth-screen auth-screen--welcome"><div className="auth-grid" /><div className="auth-content"><BrandMark /><div className="auth-emblem"><ShieldCheck size={88} strokeWidth={1.4} /><span className="emblem-node emblem-node--one" /><span className="emblem-node emblem-node--two" /><span className="emblem-node emblem-node--three" /></div><p className="auth-kicker">DINA IT OPERATIONS</p><h1>Dina</h1><p className="auth-subtitle">Manage your technology<br />with confidence.</p><button className="auth-primary-button" onClick={onStart}>Get started <ArrowUpRight size={18} /></button><span className="auth-footer">Secure workspace management</span></div></div>;
}

function Login({ onSignIn, onBack }) {
  const [showPassword, setShowPassword] = useState(false);
  return <div className="auth-screen auth-screen--login"><button className="auth-back-button" onClick={onBack}><ArrowLeft size={18} /> Back</button><div className="login-card"><BrandMark /><p className="auth-kicker">DINA</p><h1>Welcome back</h1><p className="muted-copy">Sign in to continue to Dina HQ.</p><form onSubmit={(event) => { event.preventDefault(); onSignIn(); }}><label>Email address<div className="input-wrap"><Mail size={17} /><input type="email" defaultValue="alex@dina.io" aria-label="Email address" /></div></label><label>Password<div className="input-wrap"><LockKeyhole size={17} /><input type={showPassword ? "text" : "password"} defaultValue="password123" aria-label="Password" /><button type="button" onClick={() => setShowPassword(!showPassword)} aria-label="Toggle password visibility">{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></label><div className="form-meta"><label className="remember"><input type="checkbox" defaultChecked /> <span>Remember me</span></label><button type="button" onClick={() => toast("Password reset link requested")}>Forgot password?</button></div><button className="auth-primary-button" type="submit">Sign in <ArrowUpRight size={18} /></button></form><div className="auth-divider"><span>or</span></div><button className="sso-button" onClick={() => toast("SSO sign in selected")}><Network size={17} /> Continue with SSO</button><p className="login-secure"><ShieldCheck size={14} /> Your connection is encrypted and secure</p></div></div>;
}
